...
router.post('/users/signup', async  (req, res) => {
    const { name,surname, email, password, confirm_password, dni, address, secretWord, contra } = req.body;
    const errors = [];
    if (name.length <= 0){
    ...
    if (secretWord.length <= 0){
        errors.push({text: 'Por favor ingresar una palabra secreta'});
    }
    ...
    if(errors.length > 0){
        res.render('users/signup', {errors, name, email, password, confirm_password, secretWord});
    } else {
        ...
        newUser.contra = newUser.password; 
